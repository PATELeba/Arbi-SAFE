...
function hasInjectedWeb3Key(token: string) {
  if (typeof window === "undefined" || !window.injectedWeb3) return false;

  return Object.keys(window.injectedWeb3).some((key) =>
    key.toLowerCase().includes(token.toLowerCase()),
  );
}

function hasWalletExtensionHint(wallet: WalletOption) {
  if (typeof window === "undefined") return false;
  const ethereum = window.ethereum;

  if (wallet === "okx") {
    return (
      Boolean(getInjectedProvider(ethereum, "okx")) ||
      Boolean(window.okxwallet) ||
      hasInjectedWeb3Key("okx")
    );
  }

  if (wallet === "subwallet") {
    return (
      Boolean(getInjectedProvider(ethereum, "subwallet")) ||
      Boolean(window.subwallet) ||
      hasInjectedWeb3Key("subwallet")
    );
  }

  if (wallet === "metamask") {
    return Boolean(getInjectedProvider(ethereum, "metamask"));
  }
...
const DISCONNECT_STORAGE_KEY = "arbishield-manual-disconnect";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (window.localStorage.getItem(DISCONNECT_STORAGE_KEY) === "1") return;
    ...
    void window.ethereum
      ...
      .then((accounts) => {
        ...
        if (nextAccounts.length > 0) {
          setConnectedAddress(nextAccounts[0]);
          setWallet((currentWallet) => currentWallet || nextAccounts[0]);
  ...
  const availableWallets = walletOptions.map((option) => {
    ...
    const extensionDetected = hasWalletExtensionHint(option.id);
    const installed =
      option.id === "walletconnect" ? true : Boolean(injected) || extensionDetected;
  ...
  async function connectWallet(option: WalletOption) {
    ...
    if (!provider) {
      if (hasWalletExtensionHint(option)) {
        setModalMessage(
          `${walletName} is installed but not ready in this tab yet. Open the wallet extension once, then refresh and retry.`,
        );
      } else {
        setModalMessage(`${walletName} is not detected in this browser yet. Use install, then refresh.`);
      }
      setConnectingWallet(null);
      return;
    }
    ...
    try {
      try {
        ...
        // Not all providers support explicit permissions; fall back to direct connect.
      }
      ...
      setModalMessage(`${walletName} did not return an account. Try again from the wallet extension.`);
    } catch {
  ...
  return (
    <main className="relative font-ui min-h-screen overflow-hidden bg-[#f4efe6] text-[#171411]">
        ...
        <header className="grid gap-10 border-b border-black/8 pb-10 lg:grid-cols-[1.18fr_0.82fr] lg:items-start">
          ...
          <div className="space-y-6 lg:pr-8">
            <div className="space-y-4 lg:flex lg:min-h-[24rem] lg:flex-col lg:items-center lg:justify-center lg:text-center">
              <div className="font-headline max-w-4xl text-[2.6rem] text-[#171411] sm:text-[3.3rem] lg:text-[4.15rem]">
              ...
              <p className="max-w-2xl text-base leading-8 text-[#554f46] sm:text-lg">
                This version is built to feel less like a cold dashboard and more
                like a high-trust decision layer for Arbitrum users who need
                simpler, safer next moves.
              </p>
            </div>
          </div>
              ...
              <div className="bg-[#221d18] p-5">
                ...
                <p className="mt-3 text-sm leading-7 text-[#f6efe4]">
                  {!connectedAddress
                    ...
                    : isArbitrumOne
                      ? "Mainnet confirmed. Wallet is connected on Arbitrum One and ready for live-safe guidance."
                ...
                <div className="mt-6 grid gap-4 lg:grid-cols-[0.88fr_1.12fr]">
                  <div className="bg-[linear-gradient(160deg,#ff7b35_0%,#ffc17f_100%)] p-6 text-[#21170f] shadow-[0_16px_32px_rgba(255,123,53,0.18)]">
                    ...
                    <p className="mt-3 text-sm leading-7 text-[#593620]">
                      Run the analysis and turn this panel into your wow moment:
                      risk notes, action ideas, and a cleaner story for the
                      user.
                    </p>
                  </div>
            ...
            <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
              ...
              <div className="border border-[#c9e8df] bg-[#dff5ec] p-6 shadow-[0_24px_60px_rgba(17,153,142,0.08)]">
                ...
                <p className="mt-3 text-sm leading-7 text-[#325a54]">
                  The positioning is now warmer and more premium: an AI layer
                  that brings order, taste, and trust to onchain decisions.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="w-full max-w-2xl border border-black/10 bg-[#f8f3ea] p-6 shadow-[0_30px_90px_rgba(23,20,17,0.18)]">
            <div className="flex items-start justify-between gap-4 border-b border-black/8 pb-5">
              <div>
                ...
                <p className="mt-3 max-w-lg text-sm leading-7 text-[#5c564d]">
                  A cleaner launch-ready wallet picker. Provider cards are
                  prepared now, and direct connection can be turned on later.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-6 border border-black/8 bg-[#fffaf2] p-4 text-sm text-[#595349]">
              {connectedAddress
                ...
                : "Tip: keep this modal as a polished preview now, then turn on real provider auth before launching the domain."}
            </div>
          </div>
        </div>
      ) : null}
